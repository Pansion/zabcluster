...
class SyncWaiter: public WaitableEvent::Waiter {
  public:
    SyncWaiter()
        : fired_(false), signaling_event_(NULL), lock_(), cv_(&lock_) {
    }
...
bool WaitableEvent::TimedWait(const int64 max_time_in_micros) {
  const bool finite_time = max_time_in_micros >= 0;
  ...
  bool timeOut = false;
  for (;;) {
    if (sw.fired() || (finite_time && timeOut)) {
    ...
    if (finite_time) {
      sw.cv()->TimedWait(max_time_in_micros);
      timeOut = true;
    } else {
      sw.cv()->Wait();
    }
...
bool WaitableEvent::SignalAll() {
  ...
  for (std::list<Waiter*>::iterator i = kernel_->waiters_.begin(); i != kernel_->waiters_.end(); ++i) {
    if ((*i)->Fire(this))
...
bool WaitableEvent::WaitableEventKernel::Dequeue(Waiter* waiter, void* tag) {
  for (std::list<Waiter*>::iterator i = waiters_.begin(); i != waiters_.end(); ++i) {
